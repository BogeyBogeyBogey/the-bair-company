...
def step_render(input_file: str, top_n: int | None, klassen: list | None,
    ...
    render_capakey = str(render_capakey or "").strip()
    if render_capakey and "CAPAKEY" in df.columns:
        before_capakey = len(df)
        df = df[df["CAPAKEY"].astype(str) == render_capakey].copy()
        log(f"  -> Mobiele veldrender: {len(df)}/{before_capakey} lead voor CAPAKEY {render_capakey}")
        if field_photo_path and len(df):
            df["field_photo_path"] = field_photo_path
            log("  -> Eigen veldfoto gekoppeld als render-input")
    else:
        try:
    ...
    try:
        ...
        if steps_enabled.get("render", True) and current_file:
            ...
            current_file = step_render(
                ...
                render_capakey=config.get("render_capakey"),
                field_photo_path=config.get("field_photo_path"),
            )
            refresh_costs()
            if is_cancelled():
                raise Exception("Geannuleerd")
...
def latest_field_source_csv(preferred: str | None = None) -> str | None:
    """Kies de beste lokale CSV voor een mobiele veldrender."""
    if preferred:
        candidate = HERE / Path(preferred).name
        if candidate.exists() and candidate.suffix.lower() == ".csv":
            return candidate.name

    patterns = [
        "facadepilot_leads_*_scored*.csv",
        "*_scored.csv",
    ...
    for pattern in patterns:
        for path in HERE.glob(pattern):
            ...
            if "_with_renders" in stem or path.name.startswith("."):
                continue
...
def start_field_render(data: dict) -> dict:
    """Start een mobiele één-adres render vanaf een goedgekeurde veldfoto."""
    from facadepilot_lead_review import update_review

    capakey = (data.get("capakey", [""])[0] or "").strip()
    address = (data.get("address", [""])[0] or "").strip()
    source_csv = (data.get("source_csv", [""])[0] or "").strip()
    note = (data.get("note", [""])[0] or "").strip()
    if not capakey:
        return {"ok": False, "error": "capakey ontbreekt"}

    photos = load_field_photo_index()
    photo = photos.get(capakey)
    if not photo:
        return {"ok": False, "error": "Koppel eerst een eigen veldfoto."}
    ...
    if field_dir not in photo_path.parents or not photo_path.exists():
        return {"ok": False, "error": "Veldfoto ontbreekt of is ongeldig."}

    csv_name = latest_field_source_csv(source_csv)
    if not csv_name:
        return {"ok": False, "error": "Geen lokale lead-CSV gevonden."}

    with state_lock:
        if pipeline_state["running"]:
            return {"ok": False, "error": "Pipeline loopt al."}
    ...
    config = {
        "mode": "field_mobile",
        "client_brand_mode": "facadepilot",
        "niscode": "",
        "input_csv": csv_name,
        "render_capakey": capakey,
        "field_photo_path": str(photo_path),
        "render_top": 1,
        "render_klassen": None,
    ...
    with state_lock:
        pipeline_state["running"] = True
        pipeline_state["start_time"] = time.time()
        pipeline_state["mode"] = "field_mobile"

    log(f"Mobiele veldrender gestart voor {address or capakey} vanaf {csv_name}")
    t = threading.Thread(target=run_pipeline, args=(config,), daemon=True)
    t.start()
    return {"ok": True, "source_csv": csv_name, "capakey": capakey, "photo": photo}


def field_mobile_html() -> str:
...
<title>FacadePilot Field</title>
<style>
*{box-sizing:border-box}
html,body{margin:0;min-height:100%;background:#070b12;color:#eef4ff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif}
body{padding:env(safe-area-inset-top) 0 env(safe-area-inset-bottom)}
button,input,textarea,select{font:inherit}
.shell{max-width:720px;margin:0 auto;min-height:100vh;background:linear-gradient(180deg,#0d1521 0%,#070b12 100%)}
.top{position:sticky;top:0;z-index:10;padding:16px 16px 12px;background:rgba(7,11,18,.92);backdrop-filter:blur(18px);border-bottom:1px solid #1e2a3a}
.kicker{color:#e9a14f;text-transform:uppercase;font-weight:900;letter-spacing:.12em;font-size:11px}
h1{margin:5px 0 4px;font-size:26px;line-height:1.05;letter-spacing:-.02em}
.sub{color:#9aa8bc;font-size:14px;line-height:1.35}
.top-actions{display:grid;grid-template-columns:1fr auto;gap:10px;margin-top:14px}
.search{width:100%;min-height:46px;border:1px solid #223044;border-radius:12px;background:#0f1724;color:#eef4ff;padding:0 13px;outline:none}
.desk{min-height:46px;border:1px solid #26354a;border-radius:12px;background:#121c2b;color:#eef4ff;font-weight:850;padding:0 14px;text-decoration:none;display:grid;place-items:center}
.tabs{display:flex;gap:8px;overflow:auto;padding:12px 16px 8px}
.tab{border:1px solid #243247;border-radius:999px;background:#0f1724;color:#aeb9c9;font-weight:850;padding:10px 13px;white-space:nowrap}
.tab.active{background:#e79a4d;color:#111827;border-color:#e79a4d}
.status{margin:8px 16px 0;padding:12px;border:1px solid #223044;border-radius:14px;background:#0e1724;color:#b8c5d7;font-size:13px;line-height:1.35}
.status strong{color:#eef4ff}
.selected{margin:14px 16px 0;padding:14px;border:1px solid #2b3b52;border-radius:16px;background:linear-gradient(135deg,#142133,#0e1724)}
.selected h2{font-size:18px;margin:0 0 5px}
.selected .meta{color:#98a6b8;font-size:13px;line-height:1.35}
.selected-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}
.btn{min-height:48px;border:1px solid #29384d;border-radius:12px;background:#121c2b;color:#eef4ff;font-weight:900;padding:0 12px}
.btn.primary{background:#e79a4d;color:#111827;border-color:#e79a4d}
.btn.blue{background:#2563eb;border-color:#3b82f6;color:white}
.btn.ghost{background:#0b111a;color:#aeb9c9}
.btn:disabled{opacity:.45}
.list{display:grid;gap:12px;padding:14px 16px 90px}
.card{border:1px solid #1f2d3f;border-radius:17px;background:#0b111a;overflow:hidden;box-shadow:0 12px 30px rgba(0,0,0,.18)}
.card.selected-card{border-color:#e79a4d;box-shadow:0 0 0 1px rgba(231,154,77,.25),0 18px 45px rgba(0,0,0,.25)}
.card-main{display:grid;grid-template-columns:96px 1fr;gap:12px;padding:12px}
.thumb{width:96px;height:96px;border-radius:14px;background:#101a28;border:1px solid #223044;overflow:hidden;display:grid;place-items:center;color:#8da0b8;font-size:12px;text-align:center}
...
.chip.warn{background:#332413;color:#f7ba62}
.actions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;padding:0 12px 12px}
.actions .btn{min-height:44px}
.note{padding:0 12px 12px}
.note textarea{width:100%;min-height:56px;border:1px solid #223044;border-radius:12px;background:#0f1724;color:#eef4ff;padding:10px;resize:vertical}
.hidden-input{position:absolute;left:-9999px;width:1px;height:1px;opacity:0}
.toast{position:fixed;left:16px;right:16px;bottom:18px;z-index:20;border:1px solid #2a3b52;border-radius:14px;background:#101a28;color:#eef4ff;padding:12px 14px;box-shadow:0 14px 42px rgba(0,0,0,.45);font-weight:800;display:none}
.toast.show{display:block}
.empty{padding:24px 16px;color:#9aa8bc;text-align:center}
</style>
</head>
<body>
<main class="shell">
  <header class="top">
    <div class="kicker">FacadePilot field</div>
    <h1>Veldfoto's</h1>
    <div class="sub">Adres kiezen, camera openen, foto goedkeuren en render starten terwijl je naar het volgende adres gaat.</div>
    <div class="top-actions">
      <input id="search" class="search" placeholder="Zoek adres, klasse, status" oninput="renderList()">
      <a class="desk" href="/">Desktop</a>
    </div>
  </header>

  <nav class="tabs">
    <button class="tab active" id="tabWork" onclick="setScope('work')">Werkvoorraad</button>
    <button class="tab" id="tabSelected" onclick="setScope('selected')">Geselecteerd</button>
    <button class="tab" id="tabPhotos" onclick="setScope('photos')">Met foto</button>
    <button class="tab" id="tabAll" onclick="setScope('all')">Alles</button>
    <button class="tab" id="sortScore" onclick="toggleSort()">Score ↓</button>
  </nav>

  <section class="status" id="fieldStatus">Laden...</section>
  <section class="selected" id="selectedPanel" hidden></section>
  <section class="list" id="leadList"></section>
</main>
...
const pct = new Intl.NumberFormat('nl-BE', {maximumFractionDigits: 1});

function prop(f, key, fallback='') { return (f.properties && f.properties[key] != null) ? f.properties[key] : fallback; }
function keyOf(f) { return String(prop(f, 'capakey') || prop(f, 'adres') || ''); }
...
function isUseful(f) {
  const a = addressOf(f).toLowerCase();
  return a && a !== 'nan' && keyOf(f);
}
function sourceCsv() {
  return geoSource && geoSource.startsWith('csv:') ? geoSource.slice(4) : '';
}
...
async function loadData() {
  const [geo, photoData] = await Promise.all([
    fetch('/api/leads_geojson').then(r => r.json()),
    fetch('/api/field_photos').then(r => r.json())
  ]);
  features = (geo.features || []).filter(isUseful);
  geoSource = geo.source || '';
  photos = photoData.photos || {};
  if (!activeKey && features.length) activeKey = keyOf(defaultList()[0] || features[0]);
  renderList();
  pollState();
  if (!pollTimer) pollTimer = setInterval(pollState, 2500);
}

function defaultList() {
  const selected = features.filter(f => reviewOf(f) === 'selected');
  if (selected.length) return selected;
  const top = features.filter(f => ['A+', 'A'].includes(clsOf(f)));
  return top.length ? top : features;
}

function visibleFeatures() {
  let base = features;
  if (scope === 'work') base = defaultList().filter(f => !hasPhoto(f));
  if (scope === 'selected') base = features.filter(f => reviewOf(f) === 'selected');
  if (scope === 'photos') base = features.filter(hasPhoto);
  const q = document.getElementById('search').value.trim().toLowerCase();
  if (q) {
    base = base.filter(f => [addressOf(f), clsOf(f), reviewOf(f), String(prop(f, 'huistype') || '')].join(' ').toLowerCase().includes(q));
  }
  return [...base].sort((a, b) => {
    if (sortBy === 'address') return addressOf(a).localeCompare(addressOf(b), 'nl-BE');
    return scoreOf(b) - scoreOf(a) || addressOf(a).localeCompare(addressOf(b), 'nl-BE');
  });
}

function setScope(next) {
  scope = next;
  for (const id of ['Work','Selected','Photos','All']) {
    document.getElementById('tab' + id).classList.toggle('active', next.toLowerCase() === id.toLowerCase());
  }
  renderList();
}

function toggleSort() {
  sortBy = sortBy === 'score' ? 'address' : 'score';
  document.getElementById('sortScore').textContent = sortBy === 'score' ? 'Score ↓' : 'A-Z';
  renderList();
}

function renderStatus() {
  const total = features.length;
  const selected = features.filter(f => reviewOf(f) === 'selected').length;
  const withPhoto = features.filter(hasPhoto).length;
  const work = defaultList().filter(f => !hasPhoto(f)).length;
  document.getElementById('fieldStatus').innerHTML =
    `<strong>${fmt.format(work)}</strong> te fotograferen · ` +
    `${fmt.format(withPhoto)} met foto · ${fmt.format(selected)} geselecteerd · bron ${escapeHtml(geoSource || 'lokaal')}`;
}

function renderSelected() {
  const f = features.find(item => keyOf(item) === activeKey);
  ...
  panel.innerHTML = `
    ...
    <div class="selected-actions">
      <button class="btn" onclick="openMaps(${j(addressOf(f))})">Maps</button>
      <button class="btn primary" onclick="document.getElementById('selectedCameraInput').click()">${hasPhoto(f) ? 'Nieuwe foto' : 'Open camera'}</button>
      <button class="btn blue" onclick="approveAndRender(${j(k)}, ${j(addressOf(f))})" ${hasPhoto(f) ? '' : 'disabled'}>Start render</button>
      <button class="btn ghost" onclick="nextLead()">Volgende</button>
    </div>`;
}
...
function renderList() {
  ...
  if (!list.length) {
    target.innerHTML = '<div class="empty">Geen adressen in deze selectie.</div>';
    return;
  }
  target.innerHTML = list.map((f, i) => {
    const k = keyOf(f);
    const photo = photos[k];
    const inputId = `mobilePhoto_${i}`;
    const noteId = `mobileNote_${i}`;
    const img = photo && photo.path
      ? `<img src="/files/${encodeURI(photo.path)}?t=${Date.now()}" alt="">`
      : `<span>Geen<br>foto</span>`;
    return `
      <article class="card ${k === activeKey ? 'selected-card' : ''}">
        <div class="card-main" onclick="selectLead(${j(k)})">
          ...
          <div>
            ...
            <div class="chips">
              ...
              <span class="chip">${escapeHtml(String(prop(f, 'huistype') || 'woning'))}</span>
            </div>
          </div>
        </div>
        <input class="hidden-input" id="${inputId}" type="file" accept="image/*" capture="environment"
          onchange="uploadCameraFile(this, ${j(k)}, ${j(addressOf(f))})">
        <div class="note"><textarea id="${noteId}" placeholder="Feedback voor Karpathy-loop, bv. oude boerderij, geen villa"></textarea></div>
        <div class="actions">
          <button class="btn" onclick="triggerCamera(${j(k)})">Camera</button>
          <button class="btn" onclick="saveDecision(${j(k)}, 'selected', document.getElementById('${noteId}').value)">Goedkeur</button>
          <button class="btn primary" onclick="startRender(${j(k)}, ${j(addressOf(f))}, document.getElementById('${noteId}').value)" ${photo ? '' : 'disabled'}>Render</button>
        </div>
      </article>`;
  }).join('');
}
...
function nextLead() {
  const list = visibleFeatures();
  const idx = list.findIndex(f => keyOf(f) === activeKey);
  const next = list[idx + 1] || list[0];
  if (next) activeKey = keyOf(next);
  renderList();
}
...
function triggerCamera(key) {
  ...
  const active = cards.find(card => card.classList.contains('selected-card'));
  const input = active ? active.querySelector('input[type=file]') : null;
  if (input) input.click();
}

async function uploadCameraFile(input, key, address) {
  const file = input.files && input.files[0];
  if (!file) return;
  const body = new FormData();
  body.append('capakey', key);
  ...
  input.value = '';
  if (!data.ok) {
    showToast(data.error || 'Upload mislukt');
    return;
  }
  photos[key] = data.photo;
  activeKey = key;
  await saveDecision(key, 'selected', 'Mobiele veldfoto gekoppeld.');
  showToast('Foto gekoppeld');
  renderList();
}

async function saveDecision(key, decision, note) {
  const body = new URLSearchParams({capakey:key, decision, note: note || ''});
  const res = await fetch('/api/lead_review', {method:'POST', body});
  const data = await res.json();
  const f = features.find(item => keyOf(item) === key);
  if (f && data.ok) f.properties.review_decision = decision;
  if (!data.ok) showToast(data.error || 'Review niet opgeslagen');
  renderList();
}

async function startRender(key, address, note) {
  if (!photos[key]) {
    showToast('Koppel eerst een eigen foto');
    return;
  }
  showToast('Render starten...');
  const body = new URLSearchParams({
    capakey:key,
    address:address || '',
    note:note || 'Mobiel goedgekeurd voor render.',
    source_csv:sourceCsv()
  });
  const res = await fetch('/api/field_start_render', {method:'POST', body});
  const data = await res.json();
  showToast(data.ok ? 'Render gestart' : (data.error || 'Render kon niet starten'));
  pollState();
}

async function approveAndRender(key, address) {
  await saveDecision(key, 'selected', 'Mobiel goedgekeurd voor render.');
  await startRender(key, address, 'Mobiel goedgekeurd voor render.');
  nextLead();
}
...
async function pollState() {
  try {
    ...
    if (s.running || s.done || s.error) {
      const step = s.current_step || (s.done ? 'klaar' : 'wacht');
      const err = s.error ? ` · fout: ${escapeHtml(s.error)}` : '';
      document.getElementById('fieldStatus').innerHTML =
        `<strong>${s.running ? 'Render loopt' : 'Pipeline klaar'}</strong> · stap ${escapeHtml(step)}${err}`;
    } else {
      renderStatus();
    }
  } catch (e) {}
...
loadData().catch(err => {
  document.getElementById('fieldStatus').textContent = 'Laden mislukt: ' + err.message;
});
</script>
</body>
</html>"""
...
<section id="hpShell" class="hp-shell" aria-label="HomePilot campagne-navigatie">
  ...
  <nav class="hp-nav" aria-label="Werkruimtes">
    ...
    <button class="hp-nav-btn" type="button" data-hp-view-button="route"><span class="hp-nav-ico">RT</span><span class="hp-nav-label">Fotoroutes</span><span class="hp-nav-count" id="hpRouteCount">0</span></button>
...
<section id="hpTopbar" class="hp-topbar">
  ...
  <div class="hp-actions">
    ...
    <button class="hp-secondary-action" type="button" onclick="window.open('/field','_blank')">Veldmodus</button>
...
async function openExternalUrl(url) {
  if (!url) return false;
  try {
    const res = await fetch('/api/open_external_url', {
      method: 'POST',
...
function isUsableAddressFeature(feature) {
  const address = featureAddress(feature).trim().toLowerCase();
  return !!address && address !== 'nan' && address !== '(geen adres)';
}
...
function classScopedMapFeatures() {
  return (_mapFeatures || []).filter(featureMatchesRenderClasses).filter(isUsableAddressFeature);
}
...
function openFieldRouteInGoogle() {
  ...
  openExternalUrl(_fieldRoute.url);
}

function openRouteStopMaps(key) {
  const feature = featureByKey(key);
  if (!feature) return alert('Adres niet gevonden in deze route.');
  openExternalUrl(googleMapsSearchUrl(feature));
}

function openRouteStopReview(key) {
  hpSetView('leads');
  setTimeout(async () => {
    const feature = await openLeadReviewByKey(key);
    if (!feature) alert('Adres niet gevonden in de reviewlijst.');
  }, 120);
}
...
function fieldPhotoDisplayName(photo) {
  if (!photo) return '';
  return photo.original_name || photo.display_name || photo.source_filename || 'Eigen foto gekoppeld';
}

function fieldPhotoThumbHtml(photo) {
  if (!photo || !photo.path) return '<span>geen foto</span>';
  return `<img src="/files/${encodeURI(photo.path)}?t=${Date.now()}" alt="${escapeHtml(fieldPhotoDisplayName(photo))}">`;
}
...
function renderFieldPhotoList() {
  ...
  target.innerHTML = features.map((feature, index) => {
    ...
    return `<div class="photo-row">
      ...
      <div class="photo-thumb">${fieldPhotoThumbHtml(photo)}</div>
...
async function uploadFieldPhotoFile(key, address, file, row, input) {
  ...
  body.set('file', file);
  const status = row ? row.querySelector('.photo-copy span') : null;
  ...
  try {
    ...
    if (row) {
      ...
      const status = row.querySelector('.photo-copy span');
      if (thumb) thumb.innerHTML = fieldPhotoThumbHtml(data.photo);
      if (status) status.textContent = `Foto gekoppeld: ${fieldPhotoDisplayName(data.photo)}`;
    }
    hpSyncTopbar();
  } catch (e) {
...
def _local_request_allowed(headers) -> bool:
    ...
    if os.environ.get("FACADEPILOT_ALLOW_NETWORK") == "1":
        return True
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if url.path == "/api/open_external_url":
            target_url = g("url")
            parsed = urlparse(target_url)
            if parsed.scheme not in ("http", "https") or not parsed.netloc:
                return self._json({"ok": False, "error": "Ongeldige URL"}, 400)
            try:
        ...
        elif url.path == "/api/field_start_render":
            try:
                result = start_field_render(data)
                self._json(result, 200 if result.get("ok") else 400)
            except Exception as e:
                self._json({"ok": False, "error": str(e)}, 500)
    ...
    def _handle_field_photo_upload(self):
        ...
        if suffix not in {".jpg", ".jpeg", ".png", ".webp", ".heic"}:
            suffix = ".jpg"
        original_filename = (filename or "").replace("\\", "/").split("/")[-1].strip() or "veldbeeld"
